...
compost_dat <- read_sheet(
  ...
  mutate(
    ...
    compost_status = factor(
      ...
      levels = c('dormant', 'steady', 'active', 'hot', 'too hot!️')
    )
  ) %>%
  filter(!drop)
...
dat %>% 
  ...
  scale_fill_viridis_d(guide = guide_legend(title = 'Compost status', reverse = TRUE), na.translate=FALSE) +
  scale_color_viridis_c() + #guide = guide_legend(title = 'High/Low Air Temp F')) +
  #guides(color = guide_legend(title = 'High/Low Air Temp F')) +
...
plot_temp <- dat %>% 
  ...
  geom_hline(yintercept = 32, linetype = 'dotted', linewidth = 0.3, color = 'lightblue') +
  geom_ribbon(aes(x = date, ymax = TMAX, ymin = TMIN), alpha = 0.5, fill = 'darkgrey') +
  geom_line(aes(group = source_compost), linewidth=0.5, color = '#696969') +
  geom_point(aes(fill = compost_status, group = source_compost), color = '#696969', shape = 21, stroke = 0.5, size = 3) +
  scale_fill_viridis_d(na.translate=FALSE,
                       guide = guide_legend(reverse = TRUE, title = 'Compost status')
                       ) +
